...
class AppItems extends Component {
  ...
  constructor(props){
    ...
    handler:props.handlerVerItems
    }
  }
  componentDidMount(){
    this.state.itemService.getItems(this.state.folderId).then(data =>this.setState({items:data}));
  }
  ...
  render(){
    ...
    return (
      ...
      <Modal isOpen={this.state.deleteModal}>
          ...
          {"   "}
          <button className="btn btn-success" onClick={()=>{this.backToFolderList()}} >Back to folders list</button>
      </div>
    );
  }
}
